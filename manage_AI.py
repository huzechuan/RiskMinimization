...
if __name__=='__main__':
    # with gm.auto_choice():
    cluster = 'AI'
    if not (os.path.exists(f'./logs/{cluster}/') and os.path.isdir(f'./logs/{cluster}/')):
        os.makedirs(f'./logs/{cluster}/', exist_ok=True)
    running = []
    while 1:
        pool = os.listdir(f'./config_file/{cluster}')
        if len(pool) == 0:
            now_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            print(f'No task now! Exit at {now_time}')
            exit()

        gm = GPUManager()
        gpu = gm.auto_choice()
        rate = 8000
        if gm.gpus[gpu]['memory.free'] >= rate:
            task = random.choice(pool)
            file = open(f'./logs/{cluster}/' + task.replace('.config', '.log'), 'w')
            free = int(rate) - 1000
            subprocess.call(f'CUDA_VISIBLE_DEVICES={gpu} python train.py --config {task} --cluster {cluster} --free {free}&', shell=True, stdout=file)
            print(f"Running {task} on gpu{gpu}.")
            running.append(task)
            time.sleep(30)
        else:
            print(f"Looking for free GPU.\nAlready running: {running}.\nStill wait running: {pool}")
            time.sleep(180)
