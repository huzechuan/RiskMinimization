...
class StackedLayer(nn.Module):
    '''A layer stacked over the layer of feature extraction, such as LSTM, GRU.

    args:
        hidden_dim: input dim size
        label_size: the size of label

    output: log probability

    '''
    def __init__(self, hidden_dim, label_size, **kwargs):
...
class CRF(nn.Module):
    ...
    def __init__(self, hidden_dim, tagset_size, tri_parameter=None, if_bias=True, **kwargs):
        super(CRF, self).__init__()
        self.tagset_size = tagset_size

        self.use_which = tri_parameter['multilinear']
        # log_info(tri_parameter['std'])
        if self.use_which == 'D-Trilinear':
            self.dense = TriLinearScore(hidden_dim,
                                        self.tagset_size,
                                        tri_parameter['tag_dim'],
                                        tri_parameter['rank'],
                                        tri_parameter['std'])
        elif self.use_which == 'TwoBilinear':
            self.dense = BiLinearScore(hidden_dim,
                                       self.tagset_size,
                                       tri_parameter['tag_dim'],
                                       tri_parameter['std'])
        elif self.use_which == 'Trilinear':
            ...
            self.dense = ThreeBiLinearScore(hidden_dim,
                                            ...
                                            tri_parameter['tag_dim'],
                                            tri_parameter['std'])
        elif self.use_which == 'D-Quadlinear':
            self.dense = QuaLinearScore(hidden_dim,
                                        ...
                                        tri_parameter['std'],
                                        tri_parameter['scale'])
        elif self.use_which == 'D-Pentalinear':
...
class QuaLinearScore(nn.Module):
    ...
    def __init__(self, wemd_size, tagset_size, temd_size=20, rank=396, std=0.1545, scale=None, **kwargs):
        """
        Args:
    ...
    def rand_init(self):
        ...
        utils.init_trans(self.tag_emd, self.scale)
        ...
        print(f'rank={self.rank} label_emb={self.temd_size} std={self.std} ' \
              f'scale={self.scale} word_size={self.wemd_size} label_sum={self.tagset_size} ')
    ...
    def forward(self, word_emd):
        ...
        prev_word_emd = word_emd[:- 1, :, :]#.clone()
        current_word = word_emd[1:, :, :]
        ...
        g0 = torch.matmul(current_word, self.U)
        g1 = torch.matmul(prev_word_emd, self.T)
...
class TriLinearScore(nn.Module):
    ...
    def __str__(self):
        return f'rank={self.rank} label_emb={self.tag_emd} std={self.std} ' \
               f'scale={self.scale} word_size={self.wemd_size} label_sum={self.tagset_size} '
